"""Interactive region selector and live calibration tool (plan section
4.10, "Provides region selection and configuration").

This is a standalone camera session, same as it always was as
tools/region_selector.py (now relocated here as the plan's
setup_wizard.py) - it needs live, exclusive access to the camera to draw
regions and preview detection, which is a genuinely different job from
gui.py's live dashboard. It does not run inside the Tkinter dashboard;
launch it separately (or via the dashboard's "Configure Regions" button,
which spawns it as its own process) before or after a monitoring session,
not while one is using the same camera device.
...
def main() -> None:
    ...
    try:
        ...
        print(
            "If the main monitoring app (or another instance of this tool) "
            "is already running against this device, stop it first - only "
            "one process can hold the camera at a time."
        )
        return
